!function(e,A){"object"==typeof exports&&"undefined"!=typeof module?A(exports,require("@angular/core"),require("@ajf/core/models"),require("@ajf/core/utils"),require("@ajf/core/image"),require("pdfmake/build/pdfmake")):"function"==typeof define&&define.amd?define("@ajf/core/reports",["exports","@angular/core","@ajf/core/models","@ajf/core/utils","@ajf/core/image","pdfmake/build/pdfmake"],A):A(((e=e||self).ajf=e.ajf||{},e.ajf.core=e.ajf.core||{},e.ajf.core.reports={}),e.ng.core,e.ajf.core.models,e.ajf.core.utils,e.ajf.core.image,e.pdfmake.build.pdfmake)}(this,(function(e,A,t,n,f,r){"use strict";
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */var a,o=function(){function e(e,A){this._cdr=e,this.el=A}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance!==e&&(this._instance=e,this._cdr.detectChanges())},enumerable:!1,configurable:!0}),e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function u(A){switch(A){case e.AjfChartType.Line:return"line";case e.AjfChartType.Bar:return"bar";case e.AjfChartType.HorizontalBar:return"horizontalBar";case e.AjfChartType.Radar:return"radar";case e.AjfChartType.Scatter:return"scatter";case e.AjfChartType.Doughnut:return"doughnut";case e.AjfChartType.Pie:return"pie";case e.AjfChartType.PolarArea:return"polarArea";case e.AjfChartType.Bubble:return"bubble";default:return"line"}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(a=e.AjfChartType||(e.AjfChartType={}))[a.Line=0]="Line",a[a.Bar=1]="Bar",a[a.HorizontalBar=2]="HorizontalBar",a[a.Radar=3]="Radar",a[a.Scatter=4]="Scatter",a[a.Doughnut=5]="Doughnut",a[a.Pie=6]="Pie",a[a.PolarArea=7]="PolarArea",a[a.Bubble=8]="Bubble",a[a.LENGTH=9]="LENGTH";var i,d,l=function(){function e(){}return e.prototype.transform=function(e,A){return A>=0&&A<e.content.length?e.content[A]:null},e}();l.decorators=[{type:A.Pipe,args:[{name:"ajfGetColumnContent"}]}],(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
i=e.AjfAggregationType||(e.AjfAggregationType={}))[i.None=0]="None",i[i.Sum=1]="Sum",i[i.Average=2]="Average",i[i.WeightedAverage=3]="WeightedAverage",i[i.LENGTH=4]="LENGTH",(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
d=e.AjfWidgetType||(e.AjfWidgetType={}))[d.Layout=0]="Layout",d[d.PageBreak=1]="PageBreak",d[d.Image=2]="Image",d[d.Text=3]="Text",d[d.Chart=4]="Chart",d[d.Table=5]="Table",d[d.Map=6]="Map",d[d.Column=7]="Column",d[d.Formula=8]="Formula",d[d.ImageContainer=9]="ImageContainer",d[d.DynamicTable=10]="DynamicTable",d[d.LENGTH=11]="LENGTH";
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var v,j,s=function(){function e(e){this._cdr=e}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance=e,this._report=null!=e?e.report:null,this._cdr.markForCheck()},enumerable:!1,configurable:!0}),Object.defineProperty(e.prototype,"report",{get:function(){return this._report},enumerable:!1,configurable:!0}),e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function m(A){return Object.assign(Object.assign({},A),{aggregation:A.aggregation||e.AjfAggregationType.None})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */s.decorators=[{type:A.Directive}],s.ctorParameters=function(){return[{type:A.ChangeDetectorRef}]},s.propDecorators={instance:[{type:A.Input}]},(v=function v(e){this.viewContainerRef=e}).decorators=[{type:A.Directive,args:[{selector:"[ajf-widget-host]"}]}],v.ctorParameters=function(){return[{type:A.ViewContainerRef}]},(j=function j(){}).decorators=[{type:A.NgModule,args:[{declarations:[l,v],exports:[l,v]}]}];var b=function(){function e(){}return e.fromJson=function(e){if(null==e.aggregation)throw new Error("Malformed aggregation");return m(Object.assign(Object.assign({},e),{aggregation:e.aggregation}))},e}(),c=function(){function A(){}return A.fromJson=function(A){if(null==A.formula||null==A.aggregation||null==A.label)throw new Error("Malformed dataset");return A.formula=A.formula instanceof Array?A.formula=A.formula.map((function(e){return t.AjfFormulaSerializer.fromJson(e)})):t.AjfFormulaSerializer.fromJson(A.formula),A.aggregation=b.fromJson(A.aggregation),
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function n(A){return Object.assign(Object.assign({},A),{aggregation:A.aggregation||m({aggregation:e.AjfAggregationType.None})})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(A)},A}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function z(e){return Object.assign(Object.assign({},e),{styles:e.styles||{},visibility:e.visibility||t.alwaysCondition()})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */var T=function(){function A(){}return A.fromJson=function(n){if(null==n.widgetType)throw new Error("Malformed widget");n.visibility=n.visibility?t.AjfConditionSerializer.fromJson(n.visibility):t.alwaysCondition(),n.styles=n.styles||{};var f=n;if(f.widgetType===e.AjfWidgetType.Layout||f.widgetType===e.AjfWidgetType.Column)return A._widgetWithContentFromJson(f);if(f.widgetType===e.AjfWidgetType.Chart||f.widgetType===e.AjfWidgetType.Table){var r=A._dataWidgetFromJson(f);if(f.widgetType===e.AjfWidgetType.Chart){var a=r;a.labels instanceof Array?a.labels.map((function(e){return t.AjfFormulaSerializer.fromJson(e)})):null!=a.labels&&(a.labels=t.AjfFormulaSerializer.fromJson(a.labels))}return r}return f.widgetType===e.AjfWidgetType.Map&&(f.coordinate=t.AjfFormulaSerializer.fromJson(f.coordinate)),f},A._dataWidgetFromJson=function(A){var t;return t=null==A.dataset?[]:A.dataset.map(A.widgetType===e.AjfWidgetType.Table?function(e){return e.map((function(e){return c.fromJson(e)}))}:function(e){return c.fromJson(e)}),Object.assign(Object.assign({},z(A)),{dataset:t})},A._widgetWithContentFromJson=function(e){var t=(e.content||[]).map((function(e){return A.fromJson(e)}));return Object.assign(Object.assign({},z(e)),{content:t})},A}(),Z=function(){function e(){}return e.fromJson=function(e){return e.content=(e.content||[]).map((function(e){return T.fromJson(e)})),Object.assign(Object.assign({},e),{content:e.content,styles:e.styles||{}})},e}(),h=function(){function e(){}return e.fromJson=function(e){return["header","footer","content"].forEach((function(A){e[A]&&(e[A]=Z.fromJson(e[A]))})),
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function A(e){return Object.assign({},e)}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(e)},e}(),g=function(){function e(e,A){this._cfr=e,this._renderer=A,this._init=!1}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance!==e&&(this._instance=e,this._init&&this._loadComponent())},enumerable:!1,configurable:!0}),e.prototype.ngOnInit=function(){this._init=!0,this._loadComponent()},e.prototype._loadComponent=function(){var e=this;if(this._init&&null!=this._instance&&null!=this.widgetHost&&this.instance.visible){var A=this.widgetHost.viewContainerRef;A.clear();var t=this.widgetsMap[this._instance.widget.widgetType];if(null!=t){var n=t.component;try{var f=this._cfr.resolveComponentFactory(n),r=A.createComponent(f).instance;Object.keys(this._instance.widget.styles).forEach((function(A){try{e._renderer.setStyle(r.el.nativeElement,A,""+e._instance.widget.styles[A])}catch(e){}})),r.instance=this._instance,t.inputs&&Object.keys(t.inputs).forEach((function(e){e in r&&(r[e]=t.inputs[e])}))}catch(e){}}}},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */g.decorators=[{type:A.Directive}],g.ctorParameters=function(){return[{type:A.ComponentFactoryResolver},{type:A.Renderer2}]},g.propDecorators={widgetHost:[{type:A.ViewChild,args:[v,{static:!0}]}],instance:[{type:A.Input}]};
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var p={},O=function(){function e(){this.componentsMap=p}return e.prototype.registerCustomWidget=function(e){var A=e.widgetType,t=e.component;if(A<100)throw new Error("Invalid custom widget type, it must be greater than 100");if(null==t)throw new Error("Invalid custom widget component");this.componentsMap[A]=e},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function P(e){var A="function"==typeof Symbol&&Symbol.iterator,t=A&&e[A],n=0;if(t)return t.call(e);if(e&&"number"==typeof e.length)return{next:function(){return e&&n>=e.length&&(e=void 0),{value:e&&e[n++],done:!e}}};throw new TypeError(A?"Object is not iterable.":"Symbol.iterator is not defined.")}function M(e,A){var t="function"==typeof Symbol&&e[Symbol.iterator];if(!t)return e;var n,f,r=t.call(e),a=[];try{for(;(void 0===A||A-- >0)&&!(n=r.next()).done;)a.push(n.value)}catch(e){f={error:e}}finally{try{n&&!n.done&&(t=r.return)&&t.call(r)}finally{if(f)throw f.error}}return a}function E(){for(var e=[],A=0;A<arguments.length;A++)e=e.concat(M(arguments[A]));return e}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function x(A,n,f){var r=n.map((function(e){return t.evaluateExpression(e.formula,f)}));switch(A.aggregation){case e.AjfAggregationType.None:if(1!==r.length)throw new Error("Invalid aggregation");return r[0];case e.AjfAggregationType.Sum:return r.map((function(e){return e.reduce((function(e,A){return e+A}),0)}));case e.AjfAggregationType.Average:case e.AjfAggregationType.WeightedAverage:return r.map((function(e){return e.reduce((function(e,A){return e+A}),0)/r.length}));default:return[]}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function W(e,A,n){return{widget:e,widgetType:e.widgetType,visible:t.evaluateExpression(e.visibility.condition,A),styles:e.styles||{}}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function y(e,A,n){var f=e.formula;if('"'===f.substr(0,1)||"'"===f.substr(0,1)){var r=f.slice(1,-1),a=null!=r&&"string"==typeof r&&r.trim().length>0?n.instant(r):r;r.length>0&&(f='"'+a+'"')}else f=null!=f&&"string"==typeof f&&f.trim().length>0?n.instant(f):f;return t.evaluateExpression(f,A)}function I(A,f,r){var a=W(A,f);if(A.widgetType===e.AjfWidgetType.Column||A.widgetType===e.AjfWidgetType.Layout){var o=A,i=a,d=[];o.content.forEach((function(e){if(null!=o.repetitions){if(i.repetitions=t.evaluateExpression(o.repetitions.formula,f),"number"==typeof i.repetitions&&i.repetitions>0)for(var A=0;A<i.repetitions;A++)d.push(I(e,Object.assign(Object.assign({},f),{$repetition:A}),r))}else d.push(I(e,f,r));i.content=d}))}else if(A.widgetType===e.AjfWidgetType.Chart){var l=A,v=a,j=(l.labels instanceof Array?l.labels:[l.labels]).map((function(e){var A=t.evaluateExpression(e.formula,f);try{A=A instanceof Array?A.map((function(e){return null!=e&&"string"==typeof e&&e.trim().length>0?r.instant(e):e})):null!=A&&"string"==typeof A&&A.trim().length>0?r.instant(A):A}catch(e){}return A}));if(v.labels=l.labels instanceof Array?j:j[0],v.datasets=l.dataset.map((function(e){var A=Object.assign(Object.assign({},e.options||{}),{data:x(e.aggregation,e.formula,f)});if(null!=e.chartType){var t=u(e.chartType);A=Object.assign(Object.assign({},A),{chartType:t,type:t})}return null!=e.options&&(A=Object.assign(Object.assign({},A),{options:e.options})),null!=e.label&&(A=Object.assign(Object.assign({},A),{label:e.label.trim().length>0?r.instant(e.label):e.label})),null!=e.datalabels&&(A.datalabels=n.deepCopy(e.datalabels)),A})),v.data={labels:v.labels,datasets:v.datasets},v.chartType=u(l.type||l.chartType),null!=l.options&&null!=l.options.plugins){var s=l.options.plugins;Object.keys(s).forEach((function(e){var A=s[e];Object.keys(A).forEach((function(e){var n=A[e];"string"!=typeof n&&null!=n&&null!=n.formula&&(A[e]=t.evaluateExpression(n.formula,f))}))}))}}else if(A.widgetType===e.AjfWidgetType.Table){var m=A,b=a;b.dataset=m.dataset.map((function(e){return e.map((function(e){return e.formula instanceof Array?e.formula.map((function(e){return y(e,f,r)})):y(e.formula,f,r)}))})),b.data=(m.dataset||[]).map((function(e){return e.map((function(e){var A="";try{A=e.formula instanceof Array?e.formula.map((function(e){return y(e,f,r)})):y(e.formula,f,r)}catch(e){}return{value:A,style:Object.assign(Object.assign({},m.cellStyles),e.style),rowspan:e.rowspan,colspan:e.colspan}}))}))}else if(A.widgetType===e.AjfWidgetType.DynamicTable){var c=A,z=a;z.dataset=c.dataset.map((function(e){return e.formula instanceof Array?e.formula.map((function(e){return y(e,f,r)})):y(e.formula,f,r)}));var T=t.evaluateExpression(c.rowDefinition.formula,f)||[];T=(T||[]).map((function(e){return e.map((function(e){var A=e.value;try{A=A instanceof Array?A.map((function(e){return null!=e&&"string"==typeof e&&e.trim().length>0?r.instant(e):e})):null!=A&&"string"==typeof A&&A.trim().length>0?r.instant(A):A}catch(e){}return Object.assign(Object.assign({},e),{value:A})}))}));var Z=(c.dataset||[]).map((function(e){var A="";try{A=e.formula instanceof Array?e.formula.map((function(e){return y(e,f,r)})):y(e.formula,f,r)}catch(e){}return{value:A,style:Object.assign(Object.assign({},c.cellStyles),e.style),rowspan:e.rowspan,colspan:e.colspan}}));z.data=E([E(Z)],T)}else if(A.widgetType===e.AjfWidgetType.Image){var h=A,g=a;h.flag&&(g.flag=t.evaluateExpression(h.flag.formula,f)),h.icon&&(g.icon=t.evaluateExpression(h.icon.formula,f)),h.url&&(g.url=t.evaluateExpression(h.url.formula,f))}else if(A.widgetType===e.AjfWidgetType.ImageContainer){var O=A,P=a;O.flags&&(P.flags=O.flags instanceof Array?O.flags.map((function(e){return t.evaluateExpression(e.formula,f)})):t.evaluateExpression(O.flags.formula,f)),O.icons&&(P.icons=O.icons instanceof Array?O.icons.map((function(e){return t.evaluateExpression(e.formula,f)})):t.evaluateExpression(O.icons.formula,f)),O.urls&&(P.urls=O.urls instanceof Array?O.urls.map((function(e){return t.evaluateExpression(e.formula,f)})):t.evaluateExpression(O.urls.formula,f))}else if(A.widgetType===e.AjfWidgetType.Text){for(var M=a,G=/\[{2}(.+?)\]{2}/g,R=[],Y=void 0,q=A.htmlText;Y=G.exec(q);){var Q=Y.index,H=Y[0].length,N=t.createFormula({formula:Y[1]});R.push({idx:Q,len:H,formula:N})}R.reverse().forEach((function(e){var A;try{A=t.evaluateExpression(e.formula.formula,f)}catch(e){A=""}q=""+q.substr(0,e.idx)+A+q.substr(e.idx+e.len)})),M.htmlText=null!=q&&q.length>0?r.instant(q):q}else if(A.widgetType===e.AjfWidgetType.Formula)a.formula=t.evaluateExpression(A.formula.formula,f);else if(A.widgetType===e.AjfWidgetType.Map)a.coordinate=t.evaluateExpression(A.coordinate.formula,f);else if(A.widgetType>100){var V=null!=p[A.widgetType]?p[A.widgetType].initInstance:null;if(null!=V)return V(a,f,r)}return a}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function G(e,A,t){var n=e.content.map((function(e){return I(e,A,t)}));return{container:e,content:n,styles:e.styles}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function R(e){var A,t,n=[];try{for(var f=P(e.content),r=f.next();!r.done;r=f.next())n.push(Y(r.value))}catch(e){A={error:e}}finally{try{r&&!r.done&&(t=f.return)&&t.call(f)}finally{if(A)throw A.error}}return new Promise((function(e){Promise.all(n).then((function(A){var t,n,f={};try{for(var r=P(A),a=r.next();!a.done;a=r.next()){var o=a.value;f=Object.assign(Object.assign({},f),o)}}catch(e){t={error:e}}finally{try{a&&!a.done&&(n=r.return)&&n.call(r)}finally{if(t)throw t.error}}e(f)}))}))}function Y(A){switch(A.widgetType){case e.AjfWidgetType.Layout:case e.AjfWidgetType.Column:return R(A);case e.AjfWidgetType.Image:var t=A;if(t.widget.imageType!==f.AjfImageType.Image)break;return new Promise((function(e){var A=new XMLHttpRequest;A.onerror=function(){return e({})},A.onload=function(){var n=new FileReader;n.onerror=function(){return e({})},n.onloadend=function(){var A=n.result;if(A.startsWith("data:image")){var f={};f[t.url]=A,e(f)}else e({})},n.readAsDataURL(A.response)},A.open("GET",t.url),A.responseType="blob",A.send()}))}return new Promise((function(e){return e({})}))}var q={"roboto-all-400-normal.woff":"","roboto-all-400-italic.woff":"","roboto-all-500-normal.woff":"","roboto-all-500-italic.woff":""},Q={Roboto:{normal:"roboto-all-400-normal.woff",bold:"roboto-all-500-normal.woff",italics:"roboto-all-400-italic.woff",bolditalics:"roboto-all-500-italic.woff"}};function H(e,A){return new Promise((function(t){(function n(e){var A=[];return null!=e.header&&A.push(R(e.header)),null!=e.content&&A.push(R(e.content)),null!=e.footer&&A.push(R(e.footer)),new Promise((function(e){Promise.all(A).then((function(A){var t,n,f={};try{for(var r=P(A),a=r.next();!a.done;a=r.next()){var o=a.value;f=Object.assign(Object.assign({},f),o)}}catch(e){t={error:e}}finally{try{a&&!a.done&&(n=r.return)&&n.call(r)}finally{if(t)throw t.error}}e(f)}))}))})(e).then((function(n){var f=515.28;"landscape"===A&&(f=761.89);var a=function o(e,A,t){var n=[];return null!=e.header&&n.push(N(e.header,A,t)),null!=e.content&&n.push(N(e.content,A,t)),null!=e.footer&&n.push(N(e.footer,A,t)),{content:{stack:n}}}(e,n,f);a.pageOrientation=A,t(r.createPdf(a,void 0,Q,q))}))}))}function N(A,t,n){return{stack:A.content.map((function(A){return function A(t,n,r){switch(t.widget.widgetType){case e.AjfWidgetType.Layout:return function a(e,t,n){for(var f=E(e.widget.columns);f.length<e.content.length;)f.push(1);for(var r=n/(f.length||1),a=[],o=0;o<e.content.length;o++){var u=A(e.content[o],t,r);null==u.stack&&(u={stack:[u]}),u.width=-1===f[o]?"auto":"*",a.push(u)}return{columns:a}}(t,n,r);case e.AjfWidgetType.PageBreak:return{text:"",pageBreak:"after"};case e.AjfWidgetType.Image:return function o(e,A,t){if(e.widget.imageType!==f.AjfImageType.Image)return{text:""};var n=A[e.url];return null==n?{text:""}:{image:n,width:t,margin:[0,0,0,10]}}(t,n,r);case e.AjfWidgetType.Text:return function u(e){var A,t={text:(A=e.htmlText,A.replace(/<\/?[^>]+(>|$)/g,"")),margin:[0,0,0,10]};return e.htmlText.startsWith("<h1>")?(t.fontSize=20,t.margin=[0,10,0,10]):e.htmlText.startsWith("<h2>")&&(t.fontSize=15,t.margin=[0,5,0,10]),t}(t);case e.AjfWidgetType.Chart:var i=null==t.canvasDataUrl?"":t.canvasDataUrl();return""===i?{text:"[chart with no attached canvas]"}:{image:i,width:r,margin:[0,0,0,10]};case e.AjfWidgetType.Table:return function d(e){if(null==e.data||0===e.data.length)return{text:""};for(var A=[],t=0;t<e.data.length;t++){for(var n=e.data[t],f=[],r=0;r<n.length;r++){var a=n[r];f.push({text:e.dataset[t][r],colSpan:a.colspan});for(var o=1;o<(a.colspan||1);o++)f.push({text:""})}A.push(f)}return{table:{headerRows:0,body:A},margin:[0,0,0,10]}}(t);case e.AjfWidgetType.Column:return{stack:t.content.map((function(e){return A(e,n,r)}))};case e.AjfWidgetType.Formula:return{text:t.formula,margin:[0,0,0,10]};default:return{text:""}}}(A,t,n)}))}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
e.AjfAggregationSerializer=b,e.AjfBaseWidgetComponent=o,e.AjfDatasetSerializer=c,e.AjfGetColumnContentPipe=l,e.AjfReportContainerSerializer=Z,e.AjfReportRenderer=s,e.AjfReportSerializer=h,e.AjfReportWidget=g,e.AjfReportsModule=j,e.AjfWidgetHost=v,e.AjfWidgetSerializer=T,e.AjfWidgetService=O,e.chartToChartJsType=u,e.createAggregation=m,e.createReportInstance=function V(e,A,n){return(e.variables||[]).forEach((function(e){A[e.name]=t.evaluateExpression(e.formula.formula,A)})),{report:e,header:e.header?G(e.header,A,n):void 0,content:e.content?G(e.content,A,n):void 0,footer:e.footer?G(e.footer,A,n):void 0,styles:e.styles||{}}},e.createReportPdf=H,e.createWidget=z,e.createWidgetInstance=W,e.openReportPdf=function X(e,A){H(e,A).then((function(e){e.download()}))},e.widgetToWidgetInstance=I,Object.defineProperty(e,"__esModule",{value:!0})}));