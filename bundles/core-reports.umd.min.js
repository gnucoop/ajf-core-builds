!function(e,A){"object"==typeof exports&&"undefined"!=typeof module?A(exports,require("@angular/core"),require("@ajf/core/common"),require("@angular/common"),require("date-fns"),require("file-saver"),require("xlsx"),require("@ajf/core/models"),require("@ajf/core/utils"),require("@ajf/core/image"),require("pdfmake/build/pdfmake")):"function"==typeof define&&define.amd?define("@ajf/core/reports",["exports","@angular/core","@ajf/core/common","@angular/common","date-fns","file-saver","xlsx","@ajf/core/models","@ajf/core/utils","@ajf/core/image","pdfmake/build/pdfmake"],A):A(((e=e||self).ajf=e.ajf||{},e.ajf.core=e.ajf.core||{},e.ajf.core.reports={}),e.ng.core,e.ajf.core.common,e.ng.common,e.dateFns,e["file-saver"],e.xlsx,e.ajf.core.models,e.ajf.core.utils,e.ajf.core.image,e.pdfmake.build.pdfmake)}(this,(function(e,A,t,n,r,f,a,o,i,u,d){"use strict";
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */var l,s=function(){function e(e,A){this._cdr=e,this.el=A}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance!==e&&(this._instance=e,this._cdr.detectChanges())},enumerable:!1,configurable:!0}),e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function v(A){switch(A){case e.AjfChartType.Line:return"line";case e.AjfChartType.Bar:return"bar";case e.AjfChartType.HorizontalBar:return"horizontalBar";case e.AjfChartType.Radar:return"radar";case e.AjfChartType.Scatter:return"scatter";case e.AjfChartType.Doughnut:return"doughnut";case e.AjfChartType.Pie:return"pie";case e.AjfChartType.PolarArea:return"polarArea";case e.AjfChartType.Bubble:return"bubble";default:return"line"}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(l=e.AjfChartType||(e.AjfChartType={}))[l.Line=0]="Line",l[l.Bar=1]="Bar",l[l.HorizontalBar=2]="HorizontalBar",l[l.Radar=3]="Radar",l[l.Scatter=4]="Scatter",l[l.Doughnut=5]="Doughnut",l[l.Pie=6]="Pie",l[l.PolarArea=7]="PolarArea",l[l.Bubble=8]="Bubble",l[l.LENGTH=9]="LENGTH";var j,m,c=function(){function e(){}return e.prototype.transform=function(e,A){return A>=0&&A<e.content.length?e.content[A]:null},e}();c.decorators=[{type:A.Pipe,args:[{name:"ajfGetColumnContent"}]}],(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
j=e.AjfAggregationType||(e.AjfAggregationType={}))[j.None=0]="None",j[j.Sum=1]="Sum",j[j.Average=2]="Average",j[j.WeightedAverage=3]="WeightedAverage",j[j.LENGTH=4]="LENGTH",(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
m=e.AjfWidgetType||(e.AjfWidgetType={}))[m.Layout=0]="Layout",m[m.PageBreak=1]="PageBreak",m[m.Image=2]="Image",m[m.Text=3]="Text",m[m.Chart=4]="Chart",m[m.Table=5]="Table",m[m.Map=6]="Map",m[m.Column=7]="Column",m[m.Formula=8]="Formula",m[m.ImageContainer=9]="ImageContainer",m[m.DynamicTable=10]="DynamicTable",m[m.LENGTH=11]="LENGTH";
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var b=function(){function e(e){this._cdr=e}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance=e,this._report=null!=e?e.report:null,this._cdr.markForCheck()},enumerable:!1,configurable:!0}),Object.defineProperty(e.prototype,"report",{get:function(){return this._report},enumerable:!1,configurable:!0}),e}();b.decorators=[{type:A.Directive}],b.ctorParameters=function(){return[{type:A.ChangeDetectorRef}]},b.propDecorators={instance:[{type:A.Input}]};
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var z=function(){function e(){}return e.prototype.transform=function(e,A,n){return function(e,A,n){if(null==e)return"";var r=e.stringIdentifier||[];return 0===r.length?"":t.buildStringIdentifier(r,A,n)}(e,A,n)},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */z.decorators=[{type:A.Pipe,args:[{name:"ajfReportStringIdentifier"}]}];
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var p,h,g=function(){function A(){this.overlay=!0,this.enable=!1}return A.prototype.exportCsv=function(){var e=this._buildTitle(this.widgetType),A=a.utils.json_to_sheet(this._buildXlsxData()),t=a.utils.sheet_to_csv(A);f.saveAs(new Blob([t],{type:"text/csv;charset=utf-8"}),e+".csv")},A.prototype.exportXlsx=function(){var e=this._buildTitle(this.widgetType),A={};A[e]=a.utils.json_to_sheet(this._buildXlsxData());var t=a.write({Sheets:A,SheetNames:[e]},{bookType:"xlsx",type:"array"});f.saveAs(new Blob([t]),e+".xlsx")},A.prototype._buildXlsxData=function(){var A=[],t=[];switch(this.widgetType){default:case e.AjfWidgetType.Chart:this.data=this.data;var n=this.data.datasets||[];t=this.data.labels;for(var r=0;r<n.length;r++){var f={},a=n[r].data||[];f.name=n[r].label;for(var o=0;o<a.length;o++)f[t[o]]=a[o];A.push(f)}break;case e.AjfWidgetType.Table:this.data=this.data,this.data.forEach((function(e,n){var r={};0===n?t=e.map((function(e){return e.value.changingThisBreaksApplicationSecurity})):(e.forEach((function(e,A){r[t[A]]=e.value.changingThisBreaksApplicationSecurity})),A.push(r))}))}return A},A.prototype._buildTitle=function(A){return e.AjfWidgetType[A]+" "+r.format(new Date,"yyyy-MM-dd")},A}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function T(A){return Object.assign(Object.assign({},A),{aggregation:A.aggregation||e.AjfAggregationType.None})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */g.decorators=[{type:A.Component,args:[{selector:"ajf-widget-export",template:'\n    <div class="ajf-widget-wrapper">\n        <ng-content></ng-content>\n        <div  *ngIf="enable" class="ajf-export-menu" [style.display]="!overlay?\'block\':\'none\'">\n            <button (click)="exportCsv()">\n                CSV\n            </button>\n            <button (click)="exportXlsx()" mat-menu-item>\n                XLSX\n            </button>\n        </div>\n    </div>\n    ',encapsulation:A.ViewEncapsulation.None,changeDetection:A.ChangeDetectionStrategy.OnPush,styles:["ajf-widget-export{width:100%;height:inherit}ajf-widget-export .ajf-export-menu{position:absolute;right:0;top:0}ajf-widget-export .ajf-export-menu button{margin:.5em;border:none;color:#fff;background-color:#4a403f;padding:7.5px 16px;text-align:center;text-decoration:none;display:inline-block;font-size:16px;cursor:pointer}ajf-widget-export .ajf-widget-wrapper{position:relative;height:inherit}ajf-widget-export .ajf-widget-wrapper:hover .ajf-export-menu{display:block !important}\n"]}]}],g.ctorParameters=function(){return[]},g.propDecorators={widgetType:[{type:A.Input}],data:[{type:A.Input}],overlay:[{type:A.Input}],enable:[{type:A.Input}]},(p=function p(e){this.viewContainerRef=e}).decorators=[{type:A.Directive,args:[{selector:"[ajf-widget-host]"}]}],p.ctorParameters=function(){return[{type:A.ViewContainerRef}]},(h=function h(){}).decorators=[{type:A.NgModule,args:[{declarations:[c,z,p,g],imports:[n.CommonModule],exports:[c,z,p,g]}]}];var Z=function(){function e(){}return e.fromJson=function(e){if(null==e.aggregation)throw new Error("Malformed aggregation");return T(Object.assign(Object.assign({},e),{aggregation:e.aggregation}))},e}(),O=function(){function A(){}return A.fromJson=function(A){if(null==A.formula||null==A.aggregation||null==A.label)throw new Error("Malformed dataset");return A.formula=A.formula instanceof Array?A.formula=A.formula.map((function(e){return o.AjfFormulaSerializer.fromJson(e)})):o.AjfFormulaSerializer.fromJson(A.formula),A.aggregation=Z.fromJson(A.aggregation),
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function t(A){return Object.assign(Object.assign({},A),{aggregation:A.aggregation||T({aggregation:e.AjfAggregationType.None})})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(A)},A}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function x(e){return Object.assign(Object.assign({},e),{styles:e.styles||{},visibility:e.visibility||o.alwaysCondition()})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */var P=function(){function A(){}return A.fromJson=function(t){if(null==t.widgetType)throw new Error("Malformed widget");t.visibility=t.visibility?o.AjfConditionSerializer.fromJson(t.visibility):o.alwaysCondition(),t.styles=t.styles||{};var n=t;if(n.widgetType===e.AjfWidgetType.Layout||n.widgetType===e.AjfWidgetType.Column)return A._widgetWithContentFromJson(n);if(n.widgetType===e.AjfWidgetType.Chart||n.widgetType===e.AjfWidgetType.Table){var r=A._dataWidgetFromJson(n);if(n.widgetType===e.AjfWidgetType.Chart){var f=r;f.labels instanceof Array?f.labels.map((function(e){return o.AjfFormulaSerializer.fromJson(e)})):null!=f.labels&&(f.labels=o.AjfFormulaSerializer.fromJson(f.labels))}return r}return n.widgetType===e.AjfWidgetType.Map&&(n.coordinate=o.AjfFormulaSerializer.fromJson(n.coordinate)),n},A._dataWidgetFromJson=function(A){var t;return t=null==A.dataset?[]:A.dataset.map(A.widgetType===e.AjfWidgetType.Table?function(e){return e.map((function(e){return O.fromJson(e)}))}:function(e){return O.fromJson(e)}),Object.assign(Object.assign({},x(A)),{dataset:t})},A._widgetWithContentFromJson=function(e){var t=(e.content||[]).map((function(e){return A.fromJson(e)}));return Object.assign(Object.assign({},x(e)),{content:t})},A}(),y=function(){function e(){}return e.fromJson=function(e){return e.content=(e.content||[]).map((function(e){return P.fromJson(e)})),Object.assign(Object.assign({},e),{content:e.content,styles:e.styles||{}})},e}(),M=function(){function e(){}return e.fromJson=function(e){return["header","footer","content"].forEach((function(A){e[A]&&(e[A]=y.fromJson(e[A]))})),
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function A(e){return Object.assign({},e)}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(e)},e}(),E=function(){function e(e,A){this._cfr=e,this._renderer=A,this._init=!1}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance!==e&&(this._instance=e,this._init&&this._loadComponent())},enumerable:!1,configurable:!0}),e.prototype.ngOnInit=function(){this._init=!0,this._loadComponent()},e.prototype._loadComponent=function(){var e=this;if(this._init&&null!=this._instance&&null!=this.widgetHost&&this.instance.visible){var A=this.widgetHost.viewContainerRef;A.clear();var t=this.widgetsMap[this._instance.widget.widgetType];if(null!=t){var n=t.component;try{var r=this._cfr.resolveComponentFactory(n),f=A.createComponent(r).instance;Object.keys(this._instance.widget.styles).forEach((function(A){try{e._renderer.setStyle(f.el.nativeElement,A,""+e._instance.widget.styles[A])}catch(e){}})),f.instance=this._instance,t.inputs&&Object.keys(t.inputs).forEach((function(e){e in f&&(f[e]=t.inputs[e])}))}catch(e){}}}},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */E.decorators=[{type:A.Directive}],E.ctorParameters=function(){return[{type:A.ComponentFactoryResolver},{type:A.Renderer2}]},E.propDecorators={widgetHost:[{type:A.ViewChild,args:[p,{static:!0}]}],instance:[{type:A.Input}]};
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var W={},I=function(){function e(){this.componentsMap=W}return e.prototype.registerCustomWidget=function(e){var A=e.widgetType,t=e.component;if(A<100)throw new Error("Invalid custom widget type, it must be greater than 100");if(null==t)throw new Error("Invalid custom widget component");this.componentsMap[A]=e},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function G(e){var A="function"==typeof Symbol&&Symbol.iterator,t=A&&e[A],n=0;if(t)return t.call(e);if(e&&"number"==typeof e.length)return{next:function(){return e&&n>=e.length&&(e=void 0),{value:e&&e[n++],done:!e}}};throw new TypeError(A?"Object is not iterable.":"Symbol.iterator is not defined.")}function R(e,A){var t="function"==typeof Symbol&&e[Symbol.iterator];if(!t)return e;var n,r,f=t.call(e),a=[];try{for(;(void 0===A||A-- >0)&&!(n=f.next()).done;)a.push(n.value)}catch(e){r={error:e}}finally{try{n&&!n.done&&(t=f.return)&&t.call(f)}finally{if(r)throw r.error}}return a}function q(){for(var e=[],A=0;A<arguments.length;A++)e=e.concat(R(arguments[A]));return e}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function Y(A,t,n){var r=t.map((function(e){return o.evaluateExpression(e.formula,n)}));switch(A.aggregation){case e.AjfAggregationType.None:if(1!==r.length)throw new Error("Invalid aggregation");return r[0];case e.AjfAggregationType.Sum:return r.map((function(e){return e.reduce((function(e,A){return e+A}),0)}));case e.AjfAggregationType.Average:case e.AjfAggregationType.WeightedAverage:return r.map((function(e){return e.reduce((function(e,A){return e+A}),0)/r.length}));default:return[]}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function Q(e,A,t){return{widget:e,widgetType:e.widgetType,visible:o.evaluateExpression(e.visibility.condition,A),styles:e.styles||{}}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function H(e,A,t){var n=e.formula;if('"'===n.substr(0,1)||"'"===n.substr(0,1)){var r=n.slice(1,-1),f=null!=r&&"string"==typeof r&&r.trim().length>0?t.instant(r):r;r.length>0&&(n='"'+f+'"')}else n=null!=n&&"string"==typeof n&&n.trim().length>0?t.instant(n):n;return o.evaluateExpression(n,A)}function N(A,t,n){var r=Q(A,t);if(A.widgetType===e.AjfWidgetType.Column||A.widgetType===e.AjfWidgetType.Layout){var f=A,a=r,u=[];f.content.forEach((function(e){if(null!=f.repetitions){if(a.repetitions=o.evaluateExpression(f.repetitions.formula,t),"number"==typeof a.repetitions&&a.repetitions>0)for(var A=0;A<a.repetitions;A++)u.push(N(e,Object.assign(Object.assign({},t),{$repetition:A}),n))}else u.push(N(e,t,n));a.content=u}))}else if(A.widgetType===e.AjfWidgetType.Chart){var d=A,l=r,s=(d.labels instanceof Array?d.labels:[d.labels]).map((function(e){var A=o.evaluateExpression(e.formula,t);try{A=A instanceof Array?A.map((function(e){return null!=e&&"string"==typeof e&&e.trim().length>0?n.instant(e):e})):null!=A&&"string"==typeof A&&A.trim().length>0?n.instant(A):A}catch(e){}return A}));if(l.labels=d.labels instanceof Array?s:s[0],l.datasets=d.dataset.map((function(e){var A=Object.assign(Object.assign({},e.options||{}),{data:Y(e.aggregation,e.formula,t)});if(null!=e.chartType){var r=v(e.chartType);A=Object.assign(Object.assign({},A),{chartType:r,type:r})}return null!=e.options&&(A=Object.assign(Object.assign({},A),{options:e.options})),null!=e.label&&(A=Object.assign(Object.assign({},A),{label:e.label.trim().length>0?n.instant(e.label):e.label})),null!=e.datalabels&&(A.datalabels=i.deepCopy(e.datalabels)),A})),l.data={labels:l.labels,datasets:l.datasets},l.chartType=v(d.type||d.chartType),l.exportable=!(!d.exportable||!0!==d.exportable&&"true"!==d.exportable),null!=d.options&&null!=d.options.plugins){var j=d.options.plugins;Object.keys(j).forEach((function(e){var A=j[e];Object.keys(A).forEach((function(e){var n=A[e];"string"!=typeof n&&null!=n&&null!=n.formula&&(A[e]=o.evaluateExpression(n.formula,t))}))}))}}else if(A.widgetType===e.AjfWidgetType.Table){var m=A,c=r;c.dataset=m.dataset.map((function(e){return e.map((function(e){return e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)}))})),c.exportable=!(!m.exportable||!0!==m.exportable&&"true"!==m.exportable),c.data=(m.dataset||[]).map((function(e){return e.map((function(e){var A="";try{A=e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)}catch(e){}return{value:A,style:Object.assign(Object.assign({},m.cellStyles),e.style),rowspan:e.rowspan,colspan:e.colspan}}))}))}else if(A.widgetType===e.AjfWidgetType.DynamicTable){var b=A,z=r;z.dataset=b.dataset.map((function(e){return e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)})),z.exportable=!(!b.exportable||!0!==b.exportable&&"true"!==b.exportable);var p=o.evaluateExpression(b.rowDefinition.formula,t)||[];p=(p||[]).map((function(e){return e.map((function(e){var A=e.value;try{A=A instanceof Array?A.map((function(e){return null!=e&&"string"==typeof e&&e.trim().length>0?n.instant(e):e})):null!=A&&"string"==typeof A&&A.trim().length>0?n.instant(A):A}catch(e){}return Object.assign(Object.assign({},e),{value:A})}))}));var h=(b.dataset||[]).map((function(e){var A="";try{A=e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)}catch(e){}return{value:A,style:Object.assign(Object.assign({},b.cellStyles),e.style),rowspan:e.rowspan,colspan:e.colspan}}));z.data=q([q(h)],p)}else if(A.widgetType===e.AjfWidgetType.Image){var g=A,T=r;g.flag&&(T.flag=o.evaluateExpression(g.flag.formula,t)),g.icon&&(T.icon=o.evaluateExpression(g.icon.formula,t)),g.url&&(T.url=o.evaluateExpression(g.url.formula,t))}else if(A.widgetType===e.AjfWidgetType.ImageContainer){var Z=A,O=r;Z.flags&&(O.flags=Z.flags instanceof Array?Z.flags.map((function(e){return o.evaluateExpression(e.formula,t)})):o.evaluateExpression(Z.flags.formula,t)),Z.icons&&(O.icons=Z.icons instanceof Array?Z.icons.map((function(e){return o.evaluateExpression(e.formula,t)})):o.evaluateExpression(Z.icons.formula,t)),Z.urls&&(O.urls=Z.urls instanceof Array?Z.urls.map((function(e){return o.evaluateExpression(e.formula,t)})):o.evaluateExpression(Z.urls.formula,t))}else if(A.widgetType===e.AjfWidgetType.Text){for(var x=r,P=/\[{2}(.+?)\]{2}/g,y=[],M=void 0,E=A.htmlText;M=P.exec(E);){var I=M.index,G=M[0].length,R=o.createFormula({formula:M[1]});y.push({idx:I,len:G,formula:R})}y.reverse().forEach((function(e){var A;try{A=o.evaluateExpression(e.formula.formula,t)}catch(e){A=""}E=""+E.substr(0,e.idx)+A+E.substr(e.idx+e.len)})),x.htmlText=null!=E&&E.length>0?n.instant(E):E}else if(A.widgetType===e.AjfWidgetType.Formula)r.formula=o.evaluateExpression(A.formula.formula,t);else if(A.widgetType===e.AjfWidgetType.Map)r.coordinate=o.evaluateExpression(A.coordinate.formula,t);else if(A.widgetType>100){var X=null!=W[A.widgetType]?W[A.widgetType].initInstance:null;if(null!=X)return X(r,t,n)}return r}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function X(e,A,t){var n=e.content.map((function(e){return N(e,A,t)}));return{container:e,content:n,styles:e.styles}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function V(e){var A,t,n=[];try{for(var r=G(e.content),f=r.next();!f.done;f=r.next())n.push(B(f.value))}catch(e){A={error:e}}finally{try{f&&!f.done&&(t=r.return)&&t.call(r)}finally{if(A)throw A.error}}return new Promise((function(e){Promise.all(n).then((function(A){var t,n,r={};try{for(var f=G(A),a=f.next();!a.done;a=f.next()){var o=a.value;r=Object.assign(Object.assign({},r),o)}}catch(e){t={error:e}}finally{try{a&&!a.done&&(n=f.return)&&n.call(f)}finally{if(t)throw t.error}}e(r)}))}))}function B(A){switch(A.widgetType){case e.AjfWidgetType.Layout:case e.AjfWidgetType.Column:return V(A);case e.AjfWidgetType.Image:var t=A;if(t.widget.imageType!==u.AjfImageType.Image)break;return new Promise((function(e){var A=new XMLHttpRequest;A.onerror=function(){return e({})},A.onload=function(){var n=new FileReader;n.onerror=function(){return e({})},n.onloadend=function(){var A=n.result;if(A.startsWith("data:image")){var r={};r[t.url]=A,e(r)}else e({})},n.readAsDataURL(A.response)},A.open("GET",t.url),A.responseType="blob",A.send()}))}return new Promise((function(e){return e({})}))}var w={"roboto-all-400-italic.woff":"","roboto-all-400-normal.woff":"","roboto-all-500-italic.woff":"","roboto-all-500-normal.woff":""},C={Roboto:{normal:"roboto-all-400-normal.woff",bold:"roboto-all-500-normal.woff",italics:"roboto-all-400-italic.woff",bolditalics:"roboto-all-500-italic.woff"}};function K(e,A){return new Promise((function(t){(function n(e){var A=[];return null!=e.header&&A.push(V(e.header)),null!=e.content&&A.push(V(e.content)),null!=e.footer&&A.push(V(e.footer)),new Promise((function(e){Promise.all(A).then((function(A){var t,n,r={};try{for(var f=G(A),a=f.next();!a.done;a=f.next()){var o=a.value;r=Object.assign(Object.assign({},r),o)}}catch(e){t={error:e}}finally{try{a&&!a.done&&(n=f.return)&&n.call(f)}finally{if(t)throw t.error}}e(r)}))}))})(e).then((function(n){var r=515.28;"landscape"===A&&(r=761.89);var f=function a(e,A,t){var n=[];return null!=e.header&&n.push(D(e.header,A,t)),null!=e.content&&n.push(D(e.content,A,t)),null!=e.footer&&n.push(D(e.footer,A,t)),{content:{stack:n}}}(e,n,r);f.pageOrientation=A,t(d.createPdf(f,void 0,C,w))}))}))}function D(A,t,n){return{stack:A.content.map((function(A){return function A(t,n,r){switch(t.widget.widgetType){case e.AjfWidgetType.Layout:return function f(e,t,n){for(var r=q(e.widget.columns);r.length<e.content.length;)r.push(1);for(var f=n/(r.length||1),a=[],o=0;o<e.content.length;o++){var i=A(e.content[o],t,f);null==i.stack&&(i={stack:[i]}),i.width=-1===r[o]?"auto":"*",a.push(i)}return{columns:a}}(t,n,r);case e.AjfWidgetType.PageBreak:return{text:"",pageBreak:"after"};case e.AjfWidgetType.Image:return function a(e,A,t){if(e.widget.imageType!==u.AjfImageType.Image)return{text:""};var n=A[e.url];if(null==n)return{text:""};var r=e.styles.width;return"string"==typeof r&&r.endsWith("px")&&(t=Number(r.slice(0,-2))),{image:n,width:t,margin:[0,0,0,10]}}(t,n,r);case e.AjfWidgetType.Text:return function o(e){var A,t={text:(A=e.htmlText,A.replace(/<\/?[^>]+(>|$)/g,"")),margin:[0,0,0,10]};return e.htmlText.startsWith("<h1>")?(t.fontSize=20,t.margin=[0,10,0,10]):e.htmlText.startsWith("<h2>")&&(t.fontSize=15,t.margin=[0,5,0,10]),t}(t);case e.AjfWidgetType.Chart:var i=null==t.canvasDataUrl?"":t.canvasDataUrl();return""===i?{text:"[chart with no attached canvas]"}:{image:i,width:r,margin:[0,0,0,10]};case e.AjfWidgetType.Table:return function d(e){if(null==e.data||0===e.data.length)return{text:""};for(var A=[],t=0;t<e.data.length;t++){for(var n=e.data[t],r=[],f=0;f<n.length;f++){var a=n[f];r.push({text:e.dataset[t][f],colSpan:a.colspan});for(var o=1;o<(a.colspan||1);o++)r.push({text:""})}A.push(r)}return{table:{headerRows:0,body:A},margin:[0,0,0,10]}}(t);case e.AjfWidgetType.Column:return{stack:t.content.map((function(e){return A(e,n,r)}))};case e.AjfWidgetType.Formula:return{text:t.formula,margin:[0,0,0,10]};default:return{text:""}}}(A,t,n)}))}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
e.AjfAggregationSerializer=Z,e.AjfBaseWidgetComponent=s,e.AjfDatasetSerializer=O,e.AjfGetColumnContentPipe=c,e.AjfReportContainerSerializer=y,e.AjfReportRenderer=b,e.AjfReportSerializer=M,e.AjfReportStringIdentifierPipe=z,e.AjfReportWidget=E,e.AjfReportsModule=h,e.AjfWidgetExport=g,e.AjfWidgetHost=p,e.AjfWidgetSerializer=P,e.AjfWidgetService=I,e.chartToChartJsType=v,e.createAggregation=T,e.createReportInstance=function L(e,A,t){return(e.variables||[]).forEach((function(e){A[e.name]=o.evaluateExpression(e.formula.formula,A)})),{report:e,header:e.header?X(e.header,A,t):void 0,content:e.content?X(e.content,A,t):void 0,footer:e.footer?X(e.footer,A,t):void 0,styles:e.styles||{}}},e.createReportPdf=K,e.createWidget=x,e.createWidgetInstance=Q,e.openReportPdf=function F(e,A){K(e,A).then((function(e){e.open()}))},e.widgetToWidgetInstance=N,Object.defineProperty(e,"__esModule",{value:!0})}));