!function(e,A){"object"==typeof exports&&"undefined"!=typeof module?A(exports,require("@angular/core"),require("@angular/common"),require("date-fns"),require("file-saver"),require("xlsx"),require("@ajf/core/models"),require("@ajf/core/utils"),require("@ajf/core/image"),require("pdfmake/build/pdfmake")):"function"==typeof define&&define.amd?define("@ajf/core/reports",["exports","@angular/core","@angular/common","date-fns","file-saver","xlsx","@ajf/core/models","@ajf/core/utils","@ajf/core/image","pdfmake/build/pdfmake"],A):A(((e=e||self).ajf=e.ajf||{},e.ajf.core=e.ajf.core||{},e.ajf.core.reports={}),e.ng.core,e.ng.common,e.dateFns,e["file-saver"],e.xlsx,e.ajf.core.models,e.ajf.core.utils,e.ajf.core.image,e.pdfmake.build.pdfmake)}(this,(function(e,A,t,n,r,f,a,i,o,u){"use strict";var d,l="default"in r?r.default:r,s="default"in f?f.default:f,v=function(){function e(e,A){this._cdr=e,this.el=A}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance!==e&&(this._instance=e,this._cdr.detectChanges())},enumerable:!1,configurable:!0}),e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function j(A){switch(A){case e.AjfChartType.Line:return"line";case e.AjfChartType.Bar:return"bar";case e.AjfChartType.HorizontalBar:return"horizontalBar";case e.AjfChartType.Radar:return"radar";case e.AjfChartType.Scatter:return"scatter";case e.AjfChartType.Doughnut:return"doughnut";case e.AjfChartType.Pie:return"pie";case e.AjfChartType.PolarArea:return"polarArea";case e.AjfChartType.Bubble:return"bubble";default:return"line"}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
d=e.AjfChartType||(e.AjfChartType={}))[d.Line=0]="Line",d[d.Bar=1]="Bar",d[d.HorizontalBar=2]="HorizontalBar",d[d.Radar=3]="Radar",d[d.Scatter=4]="Scatter",d[d.Doughnut=5]="Doughnut",d[d.Pie=6]="Pie",d[d.PolarArea=7]="PolarArea",d[d.Bubble=8]="Bubble",d[d.LENGTH=9]="LENGTH";var m,c,b=function(){function e(){}return e.prototype.transform=function(e,A){return A>=0&&A<e.content.length?e.content[A]:null},e}();b.decorators=[{type:A.Pipe,args:[{name:"ajfGetColumnContent"}]}],(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
m=e.AjfAggregationType||(e.AjfAggregationType={}))[m.None=0]="None",m[m.Sum=1]="Sum",m[m.Average=2]="Average",m[m.WeightedAverage=3]="WeightedAverage",m[m.LENGTH=4]="LENGTH",(
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
c=e.AjfWidgetType||(e.AjfWidgetType={}))[c.Layout=0]="Layout",c[c.PageBreak=1]="PageBreak",c[c.Image=2]="Image",c[c.Text=3]="Text",c[c.Chart=4]="Chart",c[c.Table=5]="Table",c[c.Map=6]="Map",c[c.Column=7]="Column",c[c.Formula=8]="Formula",c[c.ImageContainer=9]="ImageContainer",c[c.DynamicTable=10]="DynamicTable",c[c.LENGTH=11]="LENGTH";
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var z=function(){function e(e){this._cdr=e}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance=e,this._report=null!=e?e.report:null,this._cdr.markForCheck()},enumerable:!1,configurable:!0}),Object.defineProperty(e.prototype,"report",{get:function(){return this._report},enumerable:!1,configurable:!0}),e}();function p(e){var A="function"==typeof Symbol&&Symbol.iterator,t=A&&e[A],n=0;if(t)return t.call(e);if(e&&"number"==typeof e.length)return{next:function(){return e&&n>=e.length&&(e=void 0),{value:e&&e[n++],done:!e}}};throw new TypeError(A?"Object is not iterable.":"Symbol.iterator is not defined.")}function h(e,A){var t="function"==typeof Symbol&&e[Symbol.iterator];if(!t)return e;var n,r,f=t.call(e),a=[];try{for(;(void 0===A||A-- >0)&&!(n=f.next()).done;)a.push(n.value)}catch(e){r={error:e}}finally{try{n&&!n.done&&(t=f.return)&&t.call(f)}finally{if(r)throw r.error}}return a}function g(){for(var e=[],A=0;A<arguments.length;A++)e=e.concat(h(arguments[A]));return e}z.decorators=[{type:A.Directive}],z.ctorParameters=function(){return[{type:A.ChangeDetectorRef}]},z.propDecorators={instance:[{type:A.Input}]};var T,Z,O=function(){function A(){this.overlay=!0,this.enable=!1}return A.prototype.buildCsv=function(){var A,t,n,r,f="";switch(this.widgetType){default:case e.AjfWidgetType.Chart:if(this.data=this.data,null==this.data.datasets||null==this.data.labels)return f;f=","+this.data.labels.toString()+"\n",this.data.datasets.forEach((function(e){f+=e.label+","+(e.data||[]).toString()+"\n"}));break;case e.AjfWidgetType.Table:var a="",i=0;this.data=this.data;try{for(var o=p(this.data),u=o.next();!u.done;u=o.next()){var d=u.value;f+=a;try{for(var l=(n=void 0,p(d)),s=l.next();!s.done;s=l.next()){var v=s.value;null==v.rowspan?(parseInt(v.value,10),f+=v.value+","):(i=v.rowspan,f+=v.value+",",a=",")}}catch(e){n={error:e}}finally{try{s&&!s.done&&(r=l.return)&&r.call(l)}finally{if(n)throw n.error}}","===f[f.length-1]&&(f=f.substring(0,f.length-1)),f+="\n",--i>0&&(f+=","),a=""}}catch(e){A={error:e}}finally{try{u&&!u.done&&(t=o.return)&&t.call(o)}finally{if(A)throw A.error}}}return f},A.prototype.exportCsv=function(){null!=this.widgetType&&null!=this.data&&l(new Blob([this.buildCsv()],{type:"text/csv;charset=utf-8"}),this._buildTitle(this.widgetType)+".csv")},A.prototype.buildXlsx=function(){var A=[],t=[];switch(this.widgetType){default:case e.AjfWidgetType.Chart:this.data=this.data;var n=this.data.datasets||[];t=this.data.labels;for(var r=0;r<n.length;r++){var f={},a=n[r].data||[];f.name=n[r].label;for(var i=0;i<a.length;i++)f[t[i]]=a[i];A.push(f)}break;case e.AjfWidgetType.Table:this.data=this.data,this.data.forEach((function(e,n){var r={};0===n?t=e.map((function(e){return e.value.changingThisBreaksApplicationSecurity})):(e.forEach((function(e,A){r[t[A]]=e.value.changingThisBreaksApplicationSecurity})),A.push(r))}))}return A},A.prototype.exportXlsx=function(){var e=s.utils.json_to_sheet(this.buildXlsx()),A=s.utils.book_new(),t=this._buildTitle(this.widgetType);s.utils.book_append_sheet(A,e,t),s.writeFile(A,t+".xlsx")},A.prototype._buildTitle=function(A){return e.AjfWidgetType[A]+" "+n.format(new Date,"yyyy-MM-dd")},A}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function x(A){return Object.assign(Object.assign({},A),{aggregation:A.aggregation||e.AjfAggregationType.None})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */O.decorators=[{type:A.Component,args:[{selector:"ajf-widget-export",template:'\n    <div class="ajf-widget-wrapper">\n        <ng-content></ng-content>\n        <div  *ngIf="enable" class="ajf-export-menu" [style.display]="!overlay?\'block\':\'none\'">\n            <button (click)="exportCsv()">\n                CSV\n            </button>\n            <button (click)="exportXlsx()" mat-menu-item>\n                XLSX\n            </button>\n        </div>\n    </div>\n    ',encapsulation:A.ViewEncapsulation.None,changeDetection:A.ChangeDetectionStrategy.OnPush,styles:["ajf-widget-export{width:100%;height:inherit}ajf-widget-export .ajf-export-menu{position:absolute;right:0;top:0}ajf-widget-export .ajf-export-menu button{margin:.5em;border:none;color:#fff;background-color:#4a403f;padding:7.5px 16px;text-align:center;text-decoration:none;display:inline-block;font-size:16px;cursor:pointer}ajf-widget-export .ajf-widget-wrapper{position:relative;height:inherit}ajf-widget-export .ajf-widget-wrapper:hover .ajf-export-menu{display:block !important}\n"]}]}],O.ctorParameters=function(){return[]},O.propDecorators={widgetType:[{type:A.Input}],data:[{type:A.Input}],overlay:[{type:A.Input}],enable:[{type:A.Input}]},(T=function T(e){this.viewContainerRef=e}).decorators=[{type:A.Directive,args:[{selector:"[ajf-widget-host]"}]}],T.ctorParameters=function(){return[{type:A.ViewContainerRef}]},(Z=function Z(){}).decorators=[{type:A.NgModule,args:[{declarations:[b,T,O],imports:[t.CommonModule],exports:[b,T,O]}]}];var y=function(){function e(){}return e.fromJson=function(e){if(null==e.aggregation)throw new Error("Malformed aggregation");return x(Object.assign(Object.assign({},e),{aggregation:e.aggregation}))},e}(),P=function(){function A(){}return A.fromJson=function(A){if(null==A.formula||null==A.aggregation||null==A.label)throw new Error("Malformed dataset");return A.formula=A.formula instanceof Array?A.formula=A.formula.map((function(e){return a.AjfFormulaSerializer.fromJson(e)})):a.AjfFormulaSerializer.fromJson(A.formula),A.aggregation=y.fromJson(A.aggregation),
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function t(A){return Object.assign(Object.assign({},A),{aggregation:A.aggregation||x({aggregation:e.AjfAggregationType.None})})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(A)},A}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function E(e){return Object.assign(Object.assign({},e),{styles:e.styles||{},visibility:e.visibility||a.alwaysCondition()})}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */var M=function(){function A(){}return A.fromJson=function(t){if(null==t.widgetType)throw new Error("Malformed widget");t.visibility=t.visibility?a.AjfConditionSerializer.fromJson(t.visibility):a.alwaysCondition(),t.styles=t.styles||{};var n=t;if(n.widgetType===e.AjfWidgetType.Layout||n.widgetType===e.AjfWidgetType.Column)return A._widgetWithContentFromJson(n);if(n.widgetType===e.AjfWidgetType.Chart||n.widgetType===e.AjfWidgetType.Table){var r=A._dataWidgetFromJson(n);if(n.widgetType===e.AjfWidgetType.Chart){var f=r;f.labels instanceof Array?f.labels.map((function(e){return a.AjfFormulaSerializer.fromJson(e)})):null!=f.labels&&(f.labels=a.AjfFormulaSerializer.fromJson(f.labels))}return r}return n.widgetType===e.AjfWidgetType.Map&&(n.coordinate=a.AjfFormulaSerializer.fromJson(n.coordinate)),n},A._dataWidgetFromJson=function(A){var t;return t=null==A.dataset?[]:A.dataset.map(A.widgetType===e.AjfWidgetType.Table?function(e){return e.map((function(e){return P.fromJson(e)}))}:function(e){return P.fromJson(e)}),Object.assign(Object.assign({},E(A)),{dataset:t})},A._widgetWithContentFromJson=function(e){var t=(e.content||[]).map((function(e){return A.fromJson(e)}));return Object.assign(Object.assign({},E(e)),{content:t})},A}(),W=function(){function e(){}return e.fromJson=function(e){return e.content=(e.content||[]).map((function(e){return M.fromJson(e)})),Object.assign(Object.assign({},e),{content:e.content,styles:e.styles||{}})},e}(),I=function(){function e(){}return e.fromJson=function(e){return["header","footer","content"].forEach((function(A){e[A]&&(e[A]=W.fromJson(e[A]))})),
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function A(e){return Object.assign({},e)}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */(e)},e}(),G=function(){function e(e,A){this._cfr=e,this._renderer=A,this._init=!1}return Object.defineProperty(e.prototype,"instance",{get:function(){return this._instance},set:function(e){this._instance!==e&&(this._instance=e,this._init&&this._loadComponent())},enumerable:!1,configurable:!0}),e.prototype.ngOnInit=function(){this._init=!0,this._loadComponent()},e.prototype._loadComponent=function(){var e=this;if(this._init&&null!=this._instance&&null!=this.widgetHost&&this.instance.visible){var A=this.widgetHost.viewContainerRef;A.clear();var t=this.widgetsMap[this._instance.widget.widgetType];if(null!=t){var n=t.component;try{var r=this._cfr.resolveComponentFactory(n),f=A.createComponent(r).instance;Object.keys(this._instance.widget.styles).forEach((function(A){try{e._renderer.setStyle(f.el.nativeElement,A,""+e._instance.widget.styles[A])}catch(e){}})),f.instance=this._instance,t.inputs&&Object.keys(t.inputs).forEach((function(e){e in f&&(f[e]=t.inputs[e])}))}catch(e){}}}},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */G.decorators=[{type:A.Directive}],G.ctorParameters=function(){return[{type:A.ComponentFactoryResolver},{type:A.Renderer2}]},G.propDecorators={widgetHost:[{type:A.ViewChild,args:[T,{static:!0}]}],instance:[{type:A.Input}]};
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
var R={},q=function(){function e(){this.componentsMap=R}return e.prototype.registerCustomWidget=function(e){var A=e.widgetType,t=e.component;if(A<100)throw new Error("Invalid custom widget type, it must be greater than 100");if(null==t)throw new Error("Invalid custom widget component");this.componentsMap[A]=e},e}();
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
function Y(A,t,n){var r=t.map((function(e){return a.evaluateExpression(e.formula,n)}));switch(A.aggregation){case e.AjfAggregationType.None:if(1!==r.length)throw new Error("Invalid aggregation");return r[0];case e.AjfAggregationType.Sum:return r.map((function(e){return e.reduce((function(e,A){return e+A}),0)}));case e.AjfAggregationType.Average:case e.AjfAggregationType.WeightedAverage:return r.map((function(e){return e.reduce((function(e,A){return e+A}),0)/r.length}));default:return[]}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function Q(e,A,t){return{widget:e,widgetType:e.widgetType,visible:a.evaluateExpression(e.visibility.condition,A),styles:e.styles||{}}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function H(e,A,t){var n=e.formula;if('"'===n.substr(0,1)||"'"===n.substr(0,1)){var r=n.slice(1,-1),f=null!=r&&"string"==typeof r&&r.trim().length>0?t.instant(r):r;r.length>0&&(n='"'+f+'"')}else n=null!=n&&"string"==typeof n&&n.trim().length>0?t.instant(n):n;return a.evaluateExpression(n,A)}function N(A,t,n){var r=Q(A,t);if(A.widgetType===e.AjfWidgetType.Column||A.widgetType===e.AjfWidgetType.Layout){var f=A,o=r,u=[];f.content.forEach((function(e){if(null!=f.repetitions){if(o.repetitions=a.evaluateExpression(f.repetitions.formula,t),"number"==typeof o.repetitions&&o.repetitions>0)for(var A=0;A<o.repetitions;A++)u.push(N(e,Object.assign(Object.assign({},t),{$repetition:A}),n))}else u.push(N(e,t,n));o.content=u}))}else if(A.widgetType===e.AjfWidgetType.Chart){var d=A,l=r,s=(d.labels instanceof Array?d.labels:[d.labels]).map((function(e){var A=a.evaluateExpression(e.formula,t);try{A=A instanceof Array?A.map((function(e){return null!=e&&"string"==typeof e&&e.trim().length>0?n.instant(e):e})):null!=A&&"string"==typeof A&&A.trim().length>0?n.instant(A):A}catch(e){}return A}));if(l.labels=d.labels instanceof Array?s:s[0],l.datasets=d.dataset.map((function(e){var A=Object.assign(Object.assign({},e.options||{}),{data:Y(e.aggregation,e.formula,t)});if(null!=e.chartType){var r=j(e.chartType);A=Object.assign(Object.assign({},A),{chartType:r,type:r})}return null!=e.options&&(A=Object.assign(Object.assign({},A),{options:e.options})),null!=e.label&&(A=Object.assign(Object.assign({},A),{label:e.label.trim().length>0?n.instant(e.label):e.label})),null!=e.datalabels&&(A.datalabels=i.deepCopy(e.datalabels)),A})),l.data={labels:l.labels,datasets:l.datasets},l.chartType=j(d.type||d.chartType),l.exportable=!(!d.exportable||!0!==d.exportable&&"true"!==d.exportable),null!=d.options&&null!=d.options.plugins){var v=d.options.plugins;Object.keys(v).forEach((function(e){var A=v[e];Object.keys(A).forEach((function(e){var n=A[e];"string"!=typeof n&&null!=n&&null!=n.formula&&(A[e]=a.evaluateExpression(n.formula,t))}))}))}}else if(A.widgetType===e.AjfWidgetType.Table){var m=A,c=r;c.dataset=m.dataset.map((function(e){return e.map((function(e){return e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)}))})),c.data=(m.dataset||[]).map((function(e){return e.map((function(e){var A="";try{A=e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)}catch(e){}return{value:A,style:Object.assign(Object.assign({},m.cellStyles),e.style),rowspan:e.rowspan,colspan:e.colspan}}))}))}else if(A.widgetType===e.AjfWidgetType.DynamicTable){var b=A,z=r;z.dataset=b.dataset.map((function(e){return e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)})),z.exportable=!(!b.exportable||!0!==b.exportable&&"true"!==b.exportable);var p=a.evaluateExpression(b.rowDefinition.formula,t)||[];p=(p||[]).map((function(e){return e.map((function(e){var A=e.value;try{A=A instanceof Array?A.map((function(e){return null!=e&&"string"==typeof e&&e.trim().length>0?n.instant(e):e})):null!=A&&"string"==typeof A&&A.trim().length>0?n.instant(A):A}catch(e){}return Object.assign(Object.assign({},e),{value:A})}))}));var h=(b.dataset||[]).map((function(e){var A="";try{A=e.formula instanceof Array?e.formula.map((function(e){return H(e,t,n)})):H(e.formula,t,n)}catch(e){}return{value:A,style:Object.assign(Object.assign({},b.cellStyles),e.style),rowspan:e.rowspan,colspan:e.colspan}}));z.data=g([g(h)],p)}else if(A.widgetType===e.AjfWidgetType.Image){var T=A,Z=r;T.flag&&(Z.flag=a.evaluateExpression(T.flag.formula,t)),T.icon&&(Z.icon=a.evaluateExpression(T.icon.formula,t)),T.url&&(Z.url=a.evaluateExpression(T.url.formula,t))}else if(A.widgetType===e.AjfWidgetType.ImageContainer){var O=A,x=r;O.flags&&(x.flags=O.flags instanceof Array?O.flags.map((function(e){return a.evaluateExpression(e.formula,t)})):a.evaluateExpression(O.flags.formula,t)),O.icons&&(x.icons=O.icons instanceof Array?O.icons.map((function(e){return a.evaluateExpression(e.formula,t)})):a.evaluateExpression(O.icons.formula,t)),O.urls&&(x.urls=O.urls instanceof Array?O.urls.map((function(e){return a.evaluateExpression(e.formula,t)})):a.evaluateExpression(O.urls.formula,t))}else if(A.widgetType===e.AjfWidgetType.Text){for(var y=r,P=/\[{2}(.+?)\]{2}/g,E=[],M=void 0,W=A.htmlText;M=P.exec(W);){var I=M.index,G=M[0].length,q=a.createFormula({formula:M[1]});E.push({idx:I,len:G,formula:q})}E.reverse().forEach((function(e){var A;try{A=a.evaluateExpression(e.formula.formula,t)}catch(e){A=""}W=""+W.substr(0,e.idx)+A+W.substr(e.idx+e.len)})),y.htmlText=null!=W&&W.length>0?n.instant(W):W}else if(A.widgetType===e.AjfWidgetType.Formula)r.formula=a.evaluateExpression(A.formula.formula,t);else if(A.widgetType===e.AjfWidgetType.Map)r.coordinate=a.evaluateExpression(A.coordinate.formula,t);else if(A.widgetType>100){var X=null!=R[A.widgetType]?R[A.widgetType].initInstance:null;if(null!=X)return X(r,t,n)}return r}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function X(e,A,t){var n=e.content.map((function(e){return N(e,A,t)}));return{container:e,content:n,styles:e.styles}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */function V(e){var A,t,n=[];try{for(var r=p(e.content),f=r.next();!f.done;f=r.next())n.push(B(f.value))}catch(e){A={error:e}}finally{try{f&&!f.done&&(t=r.return)&&t.call(r)}finally{if(A)throw A.error}}return new Promise((function(e){Promise.all(n).then((function(A){var t,n,r={};try{for(var f=p(A),a=f.next();!a.done;a=f.next()){var i=a.value;r=Object.assign(Object.assign({},r),i)}}catch(e){t={error:e}}finally{try{a&&!a.done&&(n=f.return)&&n.call(f)}finally{if(t)throw t.error}}e(r)}))}))}function B(A){switch(A.widgetType){case e.AjfWidgetType.Layout:case e.AjfWidgetType.Column:return V(A);case e.AjfWidgetType.Image:var t=A;if(t.widget.imageType!==o.AjfImageType.Image)break;return new Promise((function(e){var A=new XMLHttpRequest;A.onerror=function(){return e({})},A.onload=function(){var n=new FileReader;n.onerror=function(){return e({})},n.onloadend=function(){var A=n.result;if(A.startsWith("data:image")){var r={};r[t.url]=A,e(r)}else e({})},n.readAsDataURL(A.response)},A.open("GET",t.url),A.responseType="blob",A.send()}))}return new Promise((function(e){return e({})}))}var w={"roboto-all-400-italic.woff":"","roboto-all-400-normal.woff":"","roboto-all-500-italic.woff":"","roboto-all-500-normal.woff":""},C={Roboto:{normal:"roboto-all-400-normal.woff",bold:"roboto-all-500-normal.woff",italics:"roboto-all-400-italic.woff",bolditalics:"roboto-all-500-italic.woff"}};function K(e,A){return new Promise((function(t){(function n(e){var A=[];return null!=e.header&&A.push(V(e.header)),null!=e.content&&A.push(V(e.content)),null!=e.footer&&A.push(V(e.footer)),new Promise((function(e){Promise.all(A).then((function(A){var t,n,r={};try{for(var f=p(A),a=f.next();!a.done;a=f.next()){var i=a.value;r=Object.assign(Object.assign({},r),i)}}catch(e){t={error:e}}finally{try{a&&!a.done&&(n=f.return)&&n.call(f)}finally{if(t)throw t.error}}e(r)}))}))})(e).then((function(n){var r=515.28;"landscape"===A&&(r=761.89);var f=function a(e,A,t){var n=[];return null!=e.header&&n.push(D(e.header,A,t)),null!=e.content&&n.push(D(e.content,A,t)),null!=e.footer&&n.push(D(e.footer,A,t)),{content:{stack:n}}}(e,n,r);f.pageOrientation=A,t(u.createPdf(f,void 0,C,w))}))}))}function D(A,t,n){return{stack:A.content.map((function(A){return function A(t,n,r){switch(t.widget.widgetType){case e.AjfWidgetType.Layout:return function f(e,t,n){for(var r=g(e.widget.columns);r.length<e.content.length;)r.push(1);for(var f=n/(r.length||1),a=[],i=0;i<e.content.length;i++){var o=A(e.content[i],t,f);null==o.stack&&(o={stack:[o]}),o.width=-1===r[i]?"auto":"*",a.push(o)}return{columns:a}}(t,n,r);case e.AjfWidgetType.PageBreak:return{text:"",pageBreak:"after"};case e.AjfWidgetType.Image:return function a(e,A,t){if(e.widget.imageType!==o.AjfImageType.Image)return{text:""};var n=A[e.url];return null==n?{text:""}:{image:n,width:t,margin:[0,0,0,10]}}(t,n,r);case e.AjfWidgetType.Text:return function i(e){var A,t={text:(A=e.htmlText,A.replace(/<\/?[^>]+(>|$)/g,"")),margin:[0,0,0,10]};return e.htmlText.startsWith("<h1>")?(t.fontSize=20,t.margin=[0,10,0,10]):e.htmlText.startsWith("<h2>")&&(t.fontSize=15,t.margin=[0,5,0,10]),t}(t);case e.AjfWidgetType.Chart:var u=null==t.canvasDataUrl?"":t.canvasDataUrl();return""===u?{text:"[chart with no attached canvas]"}:{image:u,width:r,margin:[0,0,0,10]};case e.AjfWidgetType.Table:return function d(e){if(null==e.data||0===e.data.length)return{text:""};for(var A=[],t=0;t<e.data.length;t++){for(var n=e.data[t],r=[],f=0;f<n.length;f++){var a=n[f];r.push({text:e.dataset[t][f],colSpan:a.colspan});for(var i=1;i<(a.colspan||1);i++)r.push({text:""})}A.push(r)}return{table:{headerRows:0,body:A},margin:[0,0,0,10]}}(t);case e.AjfWidgetType.Column:return{stack:t.content.map((function(e){return A(e,n,r)}))};case e.AjfWidgetType.Formula:return{text:t.formula,margin:[0,0,0,10]};default:return{text:""}}}(A,t,n)}))}}
/**
     * @license
     * Copyright (C) Gnucoop soc. coop.
     *
     * This file is part of the Advanced JSON forms (ajf).
     *
     * Advanced JSON forms (ajf) is free software: you can redistribute it and/or
     * modify it under the terms of the GNU Affero General Public License as
     * published by the Free Software Foundation, either version 3 of the License,
     * or (at your option) any later version.
     *
     * Advanced JSON forms (ajf) is distributed in the hope that it will be useful,
     * but WITHOUT ANY WARRANTY; without even the implied warranty of
     * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
     * General Public License for more details.
     *
     * You should have received a copy of the GNU Affero General Public License
     * along with Advanced JSON forms (ajf).
     * If not, see http://www.gnu.org/licenses/.
     *
     */
e.AjfAggregationSerializer=y,e.AjfBaseWidgetComponent=v,e.AjfDatasetSerializer=P,e.AjfGetColumnContentPipe=b,e.AjfReportContainerSerializer=W,e.AjfReportRenderer=z,e.AjfReportSerializer=I,e.AjfReportWidget=G,e.AjfReportsModule=Z,e.AjfWidgetExport=O,e.AjfWidgetHost=T,e.AjfWidgetSerializer=M,e.AjfWidgetService=q,e.chartToChartJsType=j,e.createAggregation=x,e.createReportInstance=function L(e,A,t){return(e.variables||[]).forEach((function(e){A[e.name]=a.evaluateExpression(e.formula.formula,A)})),{report:e,header:e.header?X(e.header,A,t):void 0,content:e.content?X(e.content,A,t):void 0,footer:e.footer?X(e.footer,A,t):void 0,styles:e.styles||{}}},e.createReportPdf=K,e.createWidget=E,e.createWidgetInstance=Q,e.openReportPdf=function F(e,A){K(e,A).then((function(e){e.download()}))},e.widgetToWidgetInstance=N,Object.defineProperty(e,"__esModule",{value:!0})}));